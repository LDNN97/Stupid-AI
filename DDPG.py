...
parser.add_argument('--capacity', default=40, type=int) # replay buffer size
...
parser.add_argument('--max_length_of_trajectory', default=500, type=int) # num of games
parser.add_argument('--print_log', default=5, type=int)
parser.add_argument('--update_iteration', default=40, type=int)
...
class Critic(nn.Module):
    ...
    def forward(self, x, u):
        x = F.relu(self.l1(torch.cat([x, u], dim=1)))
        x = F.relu(self.l2(x))
        x = self.l3(x)
        return x
    ...
    if args.mode == 'test':
        ...
        for i in range(args.max_episode):
            ...
            for t in count():
                ...
                # add noise to action
                action = (action + max_action * np.random.normal(0, args.exploration_noise, size=env.action_space.shape[0])).clip(
